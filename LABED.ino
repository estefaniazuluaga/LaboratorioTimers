...
void setup()
{
  ...
  TCCR3A = 0;               // establecer todo el registro TCCR3A a 0
  TCCR3B = 0;               // establecer todo el registro TCCR3B a 0
  TCNT3  = 0;               // inicializar el valor del contador interno a 0. Cuenta 1 cada que pasa un ciclo.
  
  //establecer registros para incrementos de 1hz
  // Definir según el preescalador a cuantos ciclos equivale un segundo. 
  OCR3A = 15624/2;         // = (16*10^6 Hz) / (1Hz*1024) - 1 (must be <65536). Es la cantidad de ciclos que pasan en un segundo.
